...
class AnomalyDetector:
    ...
    def __init__(self, 
        ...
        self._enable_adaptive_threshold = False
        
    def enable_adaptive_threshold(self, enable: bool = True, **optimizer_kwargs):
        """
        启用/禁用自适应阈值优化
        ...
        """
        self._enable_adaptive_threshold = enable
        
        if enable:
            try:
                from adaptive_threshold_optimizer import AdaptiveThresholdOptimizer
                self.adaptive_optimizer = AdaptiveThresholdOptimizer(**optimizer_kwargs)
                
                # 从优化器获取当前最优阈值
                current_thresholds = self.adaptive_optimizer.get_current_thresholds()
                self.anomaly_threshold = current_thresholds.get('anomaly_threshold', self.anomaly_threshold)
                
                logger.info(f"已启用自适应阈值优化，当前阈值: {self.anomaly_threshold}")
            except ImportError:
                logger.error("无法导入自适应阈值优化器，请确保文件存在")
                self._enable_adaptive_threshold = False
            except Exception as e:
                logger.error(f"启用自适应阈值优化失败: {e}")
                self._enable_adaptive_threshold = False
    ...
    def predict(self, features_df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            # 自适应阈值反馈收集
            if self._enable_adaptive_threshold and self.adaptive_optimizer:
                self._collect_prediction_feedback(features_df, combined_anomaly, ensemble_scores)
    ...
    def _collect_prediction_feedback(self, features_df: pd.DataFrame, 
                                   predictions: np.ndarray, scores: np.ndarray):
        """收集预测反馈数据"""
        try:
            for i, (_, container) in enumerate(features_df.iterrows()):
                container_id = container.get('container_id', f'unknown_{i}')
                predicted_anomaly = bool(predictions[i])
                confidence_score = float(scores[i])
                
                self.adaptive_optimizer.add_detection_result(
                    container_id=container_id,
                    predicted_anomaly=predicted_anomaly,
                    confidence_score=confidence_score
                )
        except Exception as e:
            logger.error(f"收集预测反馈失败: {e}")

    def mark_false_positive(self, container_id: str, detection_time: str = None) -> bool:
        """
        标记误报
        
        Args:
            container_id: 容器ID
            detection_time: 检测时间 (ISO格式字符串)，如果为None则使用当前时间
        """
        if not self._enable_adaptive_threshold or not self.adaptive_optimizer:
            logger.warning("自适应阈值优化未启用，无法标记误报")
            return False
        
        try:
            from datetime import datetime
            timestamp = datetime.fromisoformat(detection_time) if detection_time else datetime.now()
            
            result = self.adaptive_optimizer.mark_false_positive(container_id, timestamp)
            if result:
                logger.info(f"已标记误报: {container_id}")
                # 检查是否需要更新阈值
                self._update_threshold_if_needed()
            return result
        except Exception as e:
            logger.error(f"标记误报失败: {e}")
            return False

    def mark_false_negative(self, container_id: str, actual_anomaly_time: str = None) -> bool:
        """
        标记漏报
        
        Args:
            container_id: 容器ID
            actual_anomaly_time: 实际异常时间 (ISO格式字符串)，如果为None则使用当前时间
        """
        if not self._enable_adaptive_threshold or not self.adaptive_optimizer:
            logger.warning("自适应阈值优化未启用，无法标记漏报")
            return False
        
        try:
            from datetime import datetime
            timestamp = datetime.fromisoformat(actual_anomaly_time) if actual_anomaly_time else datetime.now()
            
            result = self.adaptive_optimizer.mark_false_negative(container_id, timestamp)
            if result:
                logger.info(f"已标记漏报: {container_id}")
                # 检查是否需要更新阈值
                self._update_threshold_if_needed()
            return result
        except Exception as e:
            logger.error(f"标记漏报失败: {e}")
            return False

    def _update_threshold_if_needed(self):
        """检查并更新阈值"""
        try:
            if self.adaptive_optimizer:
                # 获取当前最优阈值
                current_thresholds = self.adaptive_optimizer.get_current_thresholds()
                new_threshold = current_thresholds.get('anomaly_threshold', self.anomaly_threshold)
                
                if abs(new_threshold - self.anomaly_threshold) > 0.01:  # 阈值变化超过0.01才更新
                    old_threshold = self.anomaly_threshold
                    self.anomaly_threshold = new_threshold
                    logger.info(f"自适应阈值已更新: {old_threshold:.3f} -> {new_threshold:.3f}")
        except Exception as e:
            logger.error(f"更新阈值失败: {e}")

    def get_adaptive_threshold_status(self) -> Dict[str, Any]:
        """
        获取自适应阈值状态
        ...
        try:
            performance_metrics = self.adaptive_optimizer.calculate_current_performance()
            recommendations = self.adaptive_optimizer.get_threshold_recommendations()
            current_thresholds = self.adaptive_optimizer.get_current_thresholds()
            ...
            return {
                ...
                'optimizer_thresholds': current_thresholds
            }
        except Exception as e:
            logger.error(f"获取自适应阈值状态失败: {e}")
            return {
                'enabled': True,
                'error': str(e)
            }

    def optimize_thresholds_now(self) -> Dict[str, Any]:
        """
        立即执行阈值优化
        ...
        try:
            result = self.adaptive_optimizer.optimize_thresholds()
            
            if result.get('status') == 'success':
                # 更新检测器的阈值
                self.anomaly_threshold = result['new_threshold']
                logger.info(f"阈值优化完成: {result['old_threshold']:.3f} -> {result['new_threshold']:.3f}")
    ...
    def generate_adaptive_threshold_report(self) -> Dict[str, Any]:
        """
        生成自适应阈值优化报告
        ...
        try:
            return self.adaptive_optimizer.generate_optimization_report()
        except Exception as e:
